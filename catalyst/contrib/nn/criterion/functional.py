...
def triplet_loss(
    embeddings: torch.Tensor, labels: torch.Tensor, margin: float = 0.3, eps: float = 1e-8
) -> torch.Tensor:
    ...
    num_positive_triplets = torch.gt(triplet_loss_value, eps).sum().float()
    triplet_loss_value = triplet_loss_value.sum() / (num_positive_triplets + eps)
...
def margin_loss(
    ...
    return loss.sum() / (skip_mask.sum() + eps)
