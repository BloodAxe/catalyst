...
class BatchTransformCallback(Callback):
    ...
    def __init__(
        ...
        self._process_input = (
            self._process_input_tuple if self.additional_input is not None else self._process_input_tensor
        )

        self.output_key = output_key or input_key
        self.additional_output = additional_output_key or self.additional_input
        self._process_output = (
            self._process_output_tuple if self.additional_output is not None else self._process_output_tensor
        )

        transforms: Sequence[nn.Module] = [item for item in transform]
        assert all(isinstance(t, nn.Module) for t in transforms), "`nn.Module` should be a base class for transforms"
    ...
    def _process_input_tuple(self, input_: dict) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def _process_output_tuple(self, runner: "IRunner", batch: Tuple[torch.Tensor, torch.Tensor]) -> None:
