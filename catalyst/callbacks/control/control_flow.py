        ...
        for loader, epochs in loaders.items():
            if isinstance(epochs, (int, float)):
                ...
                try:
                    ...
                    raise ValueError(
                        "'ignore_list' should be a dict where " "keys is a int/float/List[int]/Tuple[int]!"
                    )
        if reverse_condition:
...
def _filter_fn_from_arg(filter_fn: Union[str, FILTER_FN]) -> FILTER_FN:
    ...
    if filter_fn.__code__.co_argcount != 3:
        raise ValueError("Filter function should have three arguments - " "'stage', 'epoch' and 'loader'!")
    return filter_fn
