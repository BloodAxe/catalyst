...
class PeriodicLoaderCallback(Callback):
    ...
    def __init__(self, **kwargs):
        ...
        for loader, period in kwargs.items():
            if not isinstance(period, (int, float)):
                raise TypeError("Expected loader period type is int/float " f"but got {type(period)}!")
            period = int(period)
            if period < 0:
    ...
    def on_stage_start(self, runner: "IRunner") -> None:
        ...
        if self.loader_periods.get(runner.valid_loader, 1) < 1:
            raise ValueError(f"Period for a validation loader ('{runner.valid_loader}') " "should be > 0!")
    ...
    def on_epoch_start(self, runner: "IRunner") -> None:
        ...
        runner.valid_loader = self.valid_loader if self.valid_loader in epoch_loaders else first_loader
    ...
    def on_epoch_end(self, runner: "IRunner") -> None:
        ...
        if self.valid_loader not in runner.loaders:
            runner.epoch_metrics[valid_metric_name] = float("+inf") if runner.minimize_metric else float("-inf")
