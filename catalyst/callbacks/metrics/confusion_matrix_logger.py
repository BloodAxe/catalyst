...
class ConfusionMatrixCallback(Callback):
    ...
    Args:
        targets_key: key to use from ``runner.input``, specifies our ``y_true``
        predictions_key: key to use from ``runner.output``, specifies our ``y_pred``
    ...
    def _compute_confusion_matrix(self):
        if self._mode == "tnt":
            ...
            confusion_matrix = confusion_matrix_fn(y_true=self.targets, y_pred=self.outputs)
        else:
    ...
    def on_batch_end(self, runner: "IRunner"):
        ...
        self._add_to_stats(
            ...
            runner.input[self.targets_key].detach(),
        )
