...
class KNNMetricCallback(Callback):
    ...
    def __init__(
        ...
        Args:
            targets_key: input key to get features.
        ...
        assert metric_fn in metric_fns, f"Metric function with value `{metric_fn}` not implemented"

        self.prefix = prefix
        self.features_key = targets_key
    ...
    def on_batch_end(self, runner: "IRunner") -> None:
        ...
        """
        features: torch.Tensor = runner.output[self.features_key].cpu().detach().numpy()
        targets: torch.Tensor = runner.input[self.targets_key].cpu().detach().numpy()
    ...
    def on_loader_end(self, runner: "IRunner") -> None:
        ...
        if self.num_classes == 2:
            loader_values[self.prefix] = self.metric_fn(y_true, y_pred, average="binary")
        else:
        ...
        if self.cv_loader_names is not None:
            for k, vs in self.cv_loader_names.items():
                ...
                if k not in self.sets:
                    print(f"Set `{k}` not found in the sets. " f"Please change `cv_loader_names` parameter.")
                    continue
                ...
                for v in vs:
                    ...
                    if v not in self.sets:
                        print(f"Set `{v}` not found in the sets. " f"Please change `cv_loader_names` parameter.")
                        continue
                    ...
                    if self.num_classes == 2:
                        loader_values[f"{self.prefix}"] = self.metric_fn(y_true, y_pred, average="binary")
                    else:
                        values = self.metric_fn(y_true, y_pred, average=None)
